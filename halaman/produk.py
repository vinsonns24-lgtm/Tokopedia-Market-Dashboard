...
from src.tampilan import ABU, BIRU, ORANYE, angka, rupiah
...
pilihan_provinsi = st.sidebar.multiselect(
    "Provinsi",
...
harga_min, harga_maks = st.sidebar.select_slider(
    "Rentang harga",
    options=TITIK_HARGA,
...
st.sidebar.caption(f"{angka(len(data))} dari {angka(len(df))} produk sesuai filter")

# ------------------------------------------------------------------ Halaman
st.title("Pasar Produk")
st.caption("Listing produk Tokopedia: harga, wilayah, diskon, dan kata kunci")
...
kolom1.metric("Jumlah produk", angka(len(data)))
kolom2.metric("Jumlah toko", angka(data["nama_toko"].nunique()))
kolom3.metric("Median harga", rupiah(data["harga"].median()))
kolom4.metric("Produk berdiskon", f"{(data['diskon'] > 0).mean():.0%}")
...
with tab_harga:
    ...
    st.markdown("Rentang harga per kategori. Q1 dan Q3 adalah batas bawah dan atas dari 50% produk di tengah.")
    rentang = (
    ...
    with kiri:
        ...
        grafik_diskon = (
            ...
            .encode(
                ...
                color=alt.condition(alt.datum.besar_diskon == "0%", alt.value(ABU), alt.value(BIRU)),
                tooltip=[
        ...
        st.altair_chart(grafik_diskon, width="stretch")
        st.caption(
    ...
    with kanan:
        ...
        grafik_rating = (
            ...
            .encode(
                ...
                color=alt.condition(alt.datum.rating == "5.0", alt.value(ORANYE), alt.value(BIRU)),
                tooltip=[
        ...
        st.altair_chart(grafik_rating, width="stretch")
        st.caption(
...
with tab_kata:
    ...
    if not hasil:
        ...
        batang = dasar.mark_bar(cornerRadius=4).encode(
            ...
            color=alt.Color(
                "arah:N",
                scale=alt.Scale(domain=["di atas", "di bawah"], range=[BIRU, ORANYE]),
        ...
        st.altair_chart(batang + garis, width="stretch")
        st.caption(
